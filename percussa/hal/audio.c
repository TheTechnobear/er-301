...
// Host/percussa audio ingress does not use AM335x ADC/SPI callbacks,
// but SWIG wrappers reference these symbols, so provide inert stubs.
void Adc_callback(int *samples) { (void)samples; }

void Mod0_callback(int *samples) { (void)samples; }

void Mod1_callback(int *samples) { (void)samples; }
...
static int inputChannelMap[NUM_INPUT_CHANNELS] = {
  INPUT_IN1, INPUT_IN2, INPUT_IN3, INPUT_IN4, INPUT_A1, INPUT_B1, INPUT_C1, INPUT_D1, INPUT_A2, INPUT_B2,
  INPUT_C2,  INPUT_D2,  INPUT_A3,  INPUT_B3,  INPUT_C3, INPUT_D3, INPUT_G1, INPUT_G2, INPUT_G3, INPUT_G4};
...
static void Audio_buildInputRoutingMap(void) {
  ...
  for (uint32_t logicalJack = 0; logicalJack < logicalCount; logicalJack++) {
    ...
    if (hwCh >= 0 && (uint32_t)hwCh < MAX_CAPTURE_CHANNELS && destCh >= 0 && (uint32_t)destCh < NUM_INPUT_CHANNELS) {
...
static unsigned int findDeviceByNameMatch(rtaudio_t audio, const char *needle, bool requireInput, bool requireOutput) {
  int n = rtaudio_device_count(audio);
  if (n <= 0) {
    logError("Audio: no devices available for match '%s' (api=%s).", needle ? needle : "<null>",
...
void Audio_init() {
  ...
  logInfo("Audio: %d device(s) found via %s.", n, rtaudio_api_display_name(rtaudio_current_api(local.audio)));
  for (int i = 0; i < n; i++) {
    unsigned int id = rtaudio_get_device_id(local.audio, i);
    rtaudio_device_info_t info = rtaudio_get_device_info(local.audio, id);
    logInfo("  [%d] %s  out=%d in=%d", i, info.name, info.output_channels, info.input_channels);
  }
}
...
void Audio_start(void) {
  ...
  rtaudio_error_t err =
    rtaudio_open_stream(local.audio, &outParams, local.inputChannels > 0 ? &inParams : NULL, RTAUDIO_FORMAT_SINT32,
                        currentSampleRate(), &bufferFrames, audioCallback, NULL, NULL, NULL);
  ...
  if (err != RTAUDIO_ERROR_NONE) {
    ...
    logInfo("Audio: stream started %dHz %d-frame %d-out %d-in", currentSampleRate(), bufferFrames, local.outputChannels,
            local.inputChannels);
  }
}
...
int Audio_getRate(void) { return (int)currentSampleRate(); }
...
bool Audio_running() { return local.audio && rtaudio_is_stream_running(local.audio); }