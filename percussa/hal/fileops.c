...
#if defined(__APPLE__)
  uint64_t bytes = (uint64_t)fsinfo.f_bsize * (uint64_t)fsinfo.f_blocks;
#else
  uint64_t bytes = (uint64_t)fsinfo.f_frsize * (uint64_t)fsinfo.f_blocks;
#endif
...
#if defined(__APPLE__)
  uint64_t bytes = (uint64_t)fsinfo.f_bsize * (uint64_t)fsinfo.f_bavail;
#else
  uint64_t bytes = (uint64_t)fsinfo.f_frsize * (uint64_t)fsinfo.f_bavail;
#endif
  return (int)(bytes / 1024 / 1024);
}